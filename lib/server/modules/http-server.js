...
module.exports = function createServer(options, fsWatcher){
  ...
  function createPluginApi(server){
    return function(api, name){
      api.addSymlink = server.addSymlink;
  ...
  function processServer(server){
    ...
    server.resolveEncoding = resolveEncoding;
    server.isContentModified = isContentModified;
    server.responseToClient = responseToClient;
    server.responseError = responseError;
    ...
    server.log = function(message, verboseOnly){
      return logMsg('http', message, verboseOnly);
    };
    server.warn = function(message){
      return logWarn('http', message);
    };
    server.error = function(message){
      return logError('http', message);
    };
  }

  if (options.https) {
    var httpsOptions = require('./certs');
  ...
  plugin.extendApi(createPluginApi(server));
