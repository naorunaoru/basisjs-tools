...
var createWebServer = require('./modules/http-server');
...
function launch(config){
  ...
  var webServer = createWebServer(options, fsWatcher);
  ...
  if (rewrite)
    webServer.use(rewrite.middleware);

  webServer.use(require('./modules/http/virtualPath').createMiddleware(webServer));

  webServer.listen(options.port, function(){
    var port = this.address().port;

    console.log('Server run at ' + chalk.green((options.https ? 'https' : 'http') + '://localhost:' + port) + '\n');
  });
  ...
  if (options.sync)
  {
    var wsServer = createWsServer(webServer, options);
    require('./modules/file-sync')(wsServer, webServer, options);
    require('./modules/devtool')(wsServer, webServer, options);
  }
}
